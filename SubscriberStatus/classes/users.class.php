...
class users {
    ...
    {
        $eml = new PHPMailer();
        $user_id = "SRP" . str_pad($user_id, 7, '0', STR_PAD_LEFT);
       ...
       */
            $OK = false;
            $conn = $this->db;
            $sql = "INSERT INTO chr_srp_users (user_id, first_name, last_name, email_address, organization_name, ip_address,
...
//                exit;
            ...
            else
            {
                ...
                $eml->FromName = "Subscriber Rewards - Support Department - Error Email ";
                $eml->From = "support@cmgms.com";

                $eml->MsgHTML($stmt->error);
                $eml->Subject = "Subscriber Rewards - User submitted";
                $eml->Send();
    ...
    public function addUserFormSubmission(array $formData, $user_id, $accountType)
    {
        ...
        $OK = false;
        $conn = $this->db;
        $sql = "INSERT INTO srp_form_submissions (user_id, first_name, last_name, email_address,
...
//                exit;
        ...
        {
            $eml->AddAddress("markmatusek74@gmail.com");
            $eml->FromName = "Subscriber Rewards - Support Department - Error Email - Form Submission ";
            $eml->From = "support@cmgms.com";

            $eml->MsgHTML($stmt->error);
            $eml->Subject = "Subscriber Rewards - User submission error";
            $eml->Send();
    ...
    {
        $conn = $this->db;
        $sql = "SELECT id, REPLACE(user_id, 'SRP','') as user_id FROM `chr_srp_users` ORDER BY id DESC LIMIT 1";
    ...
    public function checkForExistingAccount($email)
    {
        $lastID = 0;
        $conn = $this->db;
        $sql = "SELECT id, REPLACE(user_id, 'SRP','') as user_id FROM `chr_srp_users` where email_address LIKE  '%" . $email . "%'";
       // print $sql . "<br />";
        $result = $conn->query($sql) or die(mysqli_error());
        while($row = $result->fetch_assoc())
        {
    ...
    public function getUserData($RecordId=NULL)
    {
        ...
        $sql = "SELECT id, user_id, first_name, last_name, email_address, organization_name, create_date, ip_address
                FROM chr_srp_users $qWhere ORDER BY last_name, first_name, email_address";
